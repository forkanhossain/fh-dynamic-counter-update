<?php
/*
 * Plugin Name:       FH Dynamic Counter Update
 * Plugin URI:        https://github.com/forkanhossain/fh-dynamic-counter-update
 ...
 * Text Domain:       dynamically-counter-update
 */
...
function fhdcu_custom_cron_intervals($schedules) {
    $schedules['every_minute'] = array(
        'interval' => 60,
        'display'  => __('Every Minute', 'dynamically-counter-update')
    );
    return $schedules;
}
...
function fhdcu_counter_settings_page() {
    if (isset($_POST['set_fhdcu_counter_value']) && check_admin_referer('fhdcu_counter_nonce')) {
        ...
        if (is_numeric($new_value)) {
            ...
            echo "<div class='updated'><p>" . esc_html__('Counter value updated!', 'dynamically-counter-update') . "</p></div>";
        } else {
            echo "<div class='error'><p>" . esc_html__('Please enter a valid number.', 'dynamically-counter-update') . "</p></div>";
        }
    }
    ...
    ?>
    <div class="wrap">
        <h1><?php esc_html_e('Counter Settings', 'dynamically-counter-update'); ?></h1>
        <form method="post" action="">
            <?php wp_nonce_field('fhdcu_counter_nonce'); ?>
            <label for="fhdcu_counter_value"><?php esc_html_e('Set Counter Value:', 'dynamically-counter-update'); ?></label>
            <input type="number" name="fhdcu_counter_value" value="<?php echo esc_attr($current_value); ?>" />
            <input type="submit" name="set_fhdcu_counter_value" value="<?php esc_attr_e('Set Counter', 'dynamically-counter-update'); ?>" class="button button-primary" />
        </form>
        <p><?php esc_html_e('Add this shortcode [fhdcu_dynamic_counter] where you want to show the counter', 'dynamically-counter-update'); ?></p>
    </div>
    <?php
}
